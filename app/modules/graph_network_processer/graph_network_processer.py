...
class GraphNetwork:
	...
	def pipeline(self):
		...
		if len(self.result)>0:
			return {"data": json.dumps(list(map(operator.itemgetter('root_symbol', 'cor_symbol'), self.result)), indent=2), "total_nodes": len(self.result), "max_depth": self.result[-1]['depth'], "elapsed_time": round(end-start, 4) }
		else:
