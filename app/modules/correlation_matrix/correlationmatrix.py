...
#This is how I think it would be nice to name the file with distance caluclation method, name of datas source, the contexts specified and time + .csv at the end of course
        df.to_csv(path_to_dataset + "/" + self.distance + "_" + self.model.model_type + "_" + dataset_filename + "_context_" + "_".join(self.context_control)+ "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") +".csv")
#______________________________________DONE_______________________________#
    ...
    def is_vector_in_model(self, vector):
    	...
    	if self.model.model_type.strip()=="bert":
